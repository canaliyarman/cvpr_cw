...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]) {
  ...
  if (!get_params(prhs, nrhs, &height, &width, &pts, &ptsenum, &thickness)) {
		mexErrMsgTxt("Scan converts a Catmull-Rom spline, given the control points and a thickness of spline, and a raster size\nSCANCR:  Use SCANCR <canvas width> <canvas height> <spline points (1:2,1:n) n is num points>=4> <thickness>");
	}
	...
	if (!canvas || !maskblock) {
		mexErrMsgTxt("SCANCR:  Out of memory creating canvas for CR spline scan conversion");
	}
