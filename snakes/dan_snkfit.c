...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray* prhs[]) {
	...
	PI=atan(1.0)*4.0;
        ...
        for(i = 0; i < 3; i++) {
          if (i==0) {
            fscanf(params_file,"%lf",&ALPHA);
          } else if (i==1) {
            fscanf(params_file,"%lf",&BETA);

          } else {
            fscanf(params_file,"%lf",&GAMMA);
          }
        }
	...
	const int* dims;
		...
		return FALSE;
                ...
                *ptscount=dims[1];
                ...
                if (dims[1]%2==1 || dims[1]<4) {
                        mexErrMsgTxt("Specify 2 x n matrix of control points\nwhere 'n' is even, and >=4");
		}
		else {
		...
		return FALSE;
                ...
                if (dims[1]!=1 || dims[0]!=1) {
                        mexErrMsgTxt("Specify 0 for a circular snake (start and end join up), or 1 for a linear one");
		}
		else {
                        if (*mxGetPr(args[2])==0)
                                *circular=0;
